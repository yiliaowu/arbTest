...
class LofValuationApp:
    ...
    def diagnose_global_environment(self):
        ...
        try:
            fx_df = pd.read_sql(
                text("SELECT date, usd_cny_mid FROM exchange_rate ORDER BY date DESC LIMIT 5"),
                self.db.get_engine(),
            )
    ...
    def diagnose_fund(self, fund):
        ...
        try:
            # 查找最近 3 个有净值的交易日
            nav_df = pd.read_sql(
                text("SELECT date, nav FROM fund_data WHERE fund_code=:code AND nav IS NOT NULL ORDER BY date DESC LIMIT 3"),
                engine,
                params={"code": code},
            )
            if nav_df.empty:
            ...
            for _, row in nav_df.iterrows():
                ...
                # 1. 查汇率
                fx = pd.read_sql(
                    text("SELECT usd_cny_mid FROM exchange_rate WHERE date=:date"),
                    engine,
                    params={"date": d},
                )
                ...
                for etf in etfs:
                    ep = pd.read_sql(
                        text("SELECT price FROM usa_etf_daily_prices WHERE symbol=:symbol AND date=:date"),
                        engine,
                        params={"symbol": etf, "date": d},
                    )
    ...
    def pre_diagnose_fund(self, fund):
        ...
        try:
            pos_df = pd.read_sql(
                text("SELECT date, position, nav as woody_nav FROM fund_daily_factors WHERE fund_code=:code ORDER BY date DESC LIMIT 1"),
                engine,
                params={"code": code},
            )
            ...
            raw_df = pd.read_sql(
                text("SELECT date, nav, price, premium FROM fund_data WHERE fund_code=:code ORDER BY date DESC LIMIT 3"),
                engine,
                params={"code": code},
            )
    ...
    def clean_duplicate_db(self):
        ...
        engine = self.db.get_engine()
        try:
            cursor.execute("SHOW TABLES LIKE 'fund_history\\_%'")
            for (table,) in cursor.fetchall():
                df = pd.read_sql(f"SELECT * FROM `{table}` ORDER BY date DESC", engine)
                if not df.empty:
